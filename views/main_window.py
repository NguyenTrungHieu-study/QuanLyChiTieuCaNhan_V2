...
class MainWindow(ctk.CTk):
    ...
    def on_search_typing(self, event):
        ...
        if not kw:
            ...
            self.filtered_transactions = [t for t in self.transactions if kw in str(t['note']).lower() or kw in str(t['category']).lower() or kw in str(t['amount'])]
        self.current_page = 1
    ...
    def delete_single_transaction(self, t):
        ...
        if mb.askyesno("Xác nhận", f"Xóa giao dịch có ID {t['id']}?"):
    ...
    def delete_category(self, name):
        ...
        if mb.askyesno("Xác nhận", f"Xóa danh mục '{name}'?"):
            if self.controller:
